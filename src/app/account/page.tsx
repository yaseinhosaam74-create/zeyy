...
import {
  ...
  updateProfile, deleteUser, reauthenticateWithPopup, type User,
} from "firebase/auth";
...
export default function AccountPage() {
  ...
  async function handleLogin() {
    ...
    setSubmitting(true);
    try { await signInWithEmailAndPassword(auth, form.email, form.password); toast.success(isAr ? "مرحباً ✓" : "Welcome back ✓"); }
    catch { toast.error(isAr ? "بيانات غير صحيحة" : "Invalid credentials"); }
    finally { setSubmitting(false); }
  }

  async function handleRegister() {
    if (!form.name || !form.email || !form.password) { toast.error(isAr ? "أكمل الحقول" : "Fill all fields"); return; }
    if (form.password.length < 6) { toast.error(isAr ? "كلمة السر 6 أحرف على الأقل" : "Min 6 chars"); return; }
    setSubmitting(true);
    try {
      ...
      toast.error(err.code === "auth/email-already-in-use" ? (isAr ? "البريد مستخدم" : "Email already in use") : (isAr ? "خطأ" : "Error"));
    } finally { setSubmitting(false); }
  }
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (!snap.exists()) {
        await setDoc(doc(db, "users", u.uid), { name: u.displayName, email: u.email, photo: u.photoURL, created_at: serverTimestamp(), role: "customer" });
      }
      toast.success(isAr ? "تم الدخول ✓" : "Signed in ✓");
  ...
  async function handleDeleteAccount() {
    if (!user) return;
    try {
      // Re-auth with Google if needed
      if (user.providerData[0]?.providerId === "google.com") {
        await reauthenticateWithPopup(user, googleProvider);
      }
      await deleteDoc(doc(db, "users", user.uid));
      await deleteUser(user);
      toast.success(isAr ? "تم حذف الحساب نهائياً" : "Account deleted");
      router.push("/");
    } catch (err: any) {
      if (err.code === "auth/requires-recent-login") {
        toast.error(isAr ? "أعد تسجيل الدخول أولاً" : "Please re-login first");
      } else {
  ...
  if (loading) return (
    <>
      ...
      <div className="min-h-screen pt-[72px] flex items-center justify-center">
        <div style={{ width: "2px", height: "48px", background: "var(--color-text)", animation: "pulse 1s ease infinite" }} />
      </div>
    </>
  );
        ...
        <div className="section-container py-16 max-w-md">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "32px" }}>
              {user.photoURL ? (
                /* Use <img> not <Image> to avoid domain config issues */
                <img
                  src={user.photoURL}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              ...
              <button onClick={() => setShowDelete(true)}
                style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "12px", letterSpacing: "0.1em", textTransform: "uppercase", color: "#c0392b", opacity: 0.65, background: "none", border: "none", cursor: "pointer" }}>
          ...
          {showDelete && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                ...
                <p style={{ fontSize: "13px", color: "var(--color-text)", opacity: 0.55, lineHeight: 1.7, marginBottom: "22px" }}>
                  {isAr
                    ? "سيتم حذف حسابك وجميع بياناتك. لا يمكن التراجع عن هذا الإجراء."
  ...
  return (
    <>
      ...
      <main className="min-h-screen pt-[72px] flex items-center justify-center px-4" dir={isAr ? "rtl" : "ltr"}>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} style={{ width: "100%", maxWidth: "360px" }}>
          ...
          {/* Tabs */}
          <div style={{ display: "flex", marginBottom: "28px", borderBottom: "1px solid var(--color-border)" }}>
            {(["login", "register"] as Tab[]).map(t_ => (
              <button key={t_} onClick={() => setTab(t_)}
                style={{
                  flex: 1, padding: "12px",
                  fontSize: "11px", letterSpacing: "0.12em", textTransform: "uppercase",
                  color: "var(--color-text)", opacity: tab === t_ ? 1 : 0.4,
                  background: "none",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            {tab === "register" && (
              <div>
                <label style={{ display: "block", fontSize: "10px", letterSpacing: "0.15em", textTransform: "uppercase", color: "var(--color-text)", opacity: 0.4, marginBottom: "6px" }}>{isAr ? "الاسم" : "Name"}</label>
            ...
            {[
              { k: "email",    l_ar: "البريد الإلكتروني", l_en: "Email",    type: "email"    },
              { k: "password", l_ar: "كلمة السر",         l_en: "Password", type: "password" },
            ].map(({ k, l_ar, l_en, type }) => (
              <div key={k}>
